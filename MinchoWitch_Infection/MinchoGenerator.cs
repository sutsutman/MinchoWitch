...
namespace MinchoWitch_Infection
{
	...
	public static class MinchoGenerator
	{
		...
		private static void CreateMinchoFilth(Pawn pawn)
		{
			for (int i = 0; i < 9; i++)
			{
				...
				if (canPlaceFilthToPos)
					FilthMaker.TryMakeFilth(pawn.Position, pawn.Map, MinchoWitch_InfectionDefOf.Mincho_Filth_BloodDef, GenText.LabelIndefinite(pawn), Rand.RangeInclusive(0, 4));
			}
		}
		...
		private static (Pawn pawn, bool isWildMan) CreateReplacedPawn(in Pawn originalPawn)
		{
			...
			MinchoWitch_InfectionDefOf.Mincho_Colonist, Faction.OfPlayer, (PawnGenerationContext)2, -1, false,
		...
		{ // TODO : 폰의 상태에 따른 민초 백스토리 설정
			...
			if (originalPawn.RaceProps.Humanlike)
			{
				if (originalPawn.IsColonist) // 정착민
				{
					...
					kind = MinchoWitch_InfectionDefOf.Mincho_Colonist;
				}
				...
				else if (originalPawn.Faction.HostileTo(Faction.OfPlayer)) // 적대
				{
					...
					faction = null;
					kind = MinchoWitch_InfectionDefOf.Mincho_WildMan;
				}
